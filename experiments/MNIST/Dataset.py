...
class Dataset:
    ...
    def get_train_batch(self, batch_index: int, batch_size: int, repeat=1, augment: bool = False) -> \
            Tuple[np.ndarray, np.ndarray, np.ndarray]:
        spikes_per_neuron, n_spikes_per_neuron, labels = self.__get_batch(self.__train_X, self.__train_labels,
                                                                          batch_index, batch_size, augment)
        return np.repeat(spikes_per_neuron, repeat, axis=0), np.repeat(n_spikes_per_neuron, repeat, axis=0), \
               np.repeat(labels, repeat, axis=0)
