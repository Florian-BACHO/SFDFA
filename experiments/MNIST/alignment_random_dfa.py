...
from online_bats.Layers import InputLayer, LIFLayer, LIFLayerRandomDFA
...
"""
Best (98.41): Numpy seed: 590679583, Cupy seed: 1997977213
Results: 98.41, 98.30, 98.27, 98.29, 98.25
"""
...
if __name__ == "__main__":
    ...
    hidden_layer = LIFLayerRandomDFA(previous_layer=input_layer, n_neurons=N_NEURONS_1, n_outputs=N_OUTPUTS,
                                     tau_s=TAU_S_1,
                                     ...
                                     name="Hidden layer 1",
                                     alpha=ALPHA, reg_factor=REG_FACTOR)
    network.add_layer(hidden_layer)

    output_layer = LIFLayerRandomDFA(previous_layer=hidden_layer, n_neurons=N_OUTPUTS, n_outputs=N_OUTPUTS,
                                     tau_s=TAU_S_OUTPUT,
    ...
    test_spike_counts_monitors = {l: SpikeCountMonitor(l.name) for l in network.layers if isinstance(l, LIFLayerRandomDFA)}
    test_silent_monitors = {l: SilentNeuronsMonitor(l.name) for l in network.layers if isinstance(l, LIFLayerRandomDFA)}
    ...
    correlation_monitor = PearsonCorrelationMonitor("Total", export_path=EXPORT_DIR / "Total correlation")
    sign_correlation_monitor = PearsonCorrelationMonitor("Sign", export_path=EXPORT_DIR / "Sign correlation")
    amplitude_correlation_monitor = PearsonCorrelationMonitor("Amplitude",
                                                              export_path=EXPORT_DIR / "Amplitude correlation")
    test_time_monitor = TimeMonitor()
    all_test_monitors = [test_loss_monitor, test_accuracy_monitor, test_learning_rate_monitor,
                         correlation_monitor, sign_correlation_monitor, amplitude_correlation_monitor]
    ...
    for epoch in range(N_TRAINING_EPOCHS):
        ...
        for batch_idx in range(N_TRAIN_BATCH):
            ...
            # Compute gradient
            avg_gradient = network.compute_avg_gradient(loss_dir_derivative, output_errors, n_out_spikes)

            # Apply step
            deltas = optimizer.step(avg_gradient)

            bats_hidden_layer.weights = hidden_layer.weights
            bats_output_layer.weights = output_layer.weights
            ...
            if training_steps % TEST_PERIOD_STEP == 0:
                ...
                correlation_monitor.add(weights, feedbacks)
                sign_correlation_monitor.add(cp.sign(weights), cp.sign(feedbacks))
                amplitude_correlation_monitor.add(cp.abs(weights), cp.abs(feedbacks))
