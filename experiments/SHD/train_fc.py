...
"""
52.58, 55.23, 54.62, 56.09, 54.62
"""

# Plot parameters
EXPORT_DIR = Path("./results/dir_dfa/" + KERNEL_TYPE + "_instable/")
...
def main(export_dir: Path):
    ...
    export_dir = export_dir / f"np_{np_seed}_cp_{cp_seed}_flr_{FEEDBACK_LEARNING_RATE}"
    ...
    for epoch in range(N_TRAINING_EPOCHS):
        ...
        print(cp.mean(hidden_layer.feedbacks))

        # Learning rate decay
        #if epoch > 0 and epoch % LR_DECAY_EPOCH == 0:
            #feedback_optimizer.learning_rate = 0.9 * feedback_optimizer.learning_rate
        ...
        for batch_idx in range(N_TRAIN_BATCH):
            ...
            #hidden_layer.feedbacks = output_layer.weights.T
            ...
            if training_steps % TEST_PERIOD_STEP == 0:
                ...
                print(f"Min feedback: {cp.min(hidden_layer.feedbacks)}, Max: {cp.max(hidden_layer.feedbacks)}")
