...
extern "C" {
    ...
    __device__ bool compute_spikes(const float c,
                                   ...
                                   int sample_idx,
                                   float perturbation) {
        float x, inside_log, inside_log_squared, tmp, du_dw, u_dir;
        ...
        while (true) {
            ...
            u_dir = 2 * dir_a / inside_log_squared - (*dir_b) / inside_log;
            post_dir_derivatives[*n_spikes] = u_dir + perturbation;

            // Update traces
            for (int i = 0; i < n_pre_neurons; i++) {
                if (trace_a[i] == 0.0)
                    continue;
                du_dw = trace_b[i] / inside_log - trace_a[i] / inside_log_squared;
                eligibility_traces[i] += du_dw;
                trace_b[i] = trace_a[i] / inside_log;
            }
    ...
    __global__ void compute_spike_times_kernel(// Parameters
        ...
        for (int i = 0; i < max_n_pre_spike; i++) {
            ...
            if (compute_spikes(c, n_spikes, out_spike_times, post_dir_derivatives, eligibility_traces,
                               a, &b, trace_a, trace_b, dir_a, &dir_b, spike_times[i], next_spike,
