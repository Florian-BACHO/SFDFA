...
def aggregate_spikes_conv(n_spikes: cp.ndarray, spike_times: cp.ndarray, dir_derivatives: cp.ndarray,
    ...
    out_dir_derivative = cp.empty((batch_size, n_post, max_pre_spike * 4), dtype=cp.int32)

    block_dim = (batch_size, 1, 1)
    grid_dim = (post_x, post_y, post_c)
    __aggregate_spikes_conv_kernel(grid_dim, block_dim, (n_spikes, spike_times, dir_derivatives, pre_shape,
                                                         max_pre_spike, out_n_spikes, out_spike_times, out_dir_derivative))
    sorted_indices = cp.argsort(out_spike_times, axis=2)
    out_spike_times = cp.take_along_axis(out_spike_times, sorted_indices, axis=2)
    out_dir_derivative = cp.take_along_axis(out_dir_derivative, sorted_indices, axis=2)

    return out_n_spikes, out_spike_times, out_dir_derivative