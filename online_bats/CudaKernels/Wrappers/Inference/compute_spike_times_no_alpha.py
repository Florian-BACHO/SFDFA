...
KERNEL_FILE = "Inference/compute_spike_times_no_alpha.cu"
KERNEL_NAME = "compute_spike_times_kernel"

__compute_spike_times_kernel = load_kernel(KERNEL_FILE, KERNEL_NAME)


def compute_spike_times_no_alpha(spike_indices: cp.ndarray, spike_times: cp.ndarray,
    ...
    last_dt_dw = cp.zeros(grad_shape, dtype=cp.float32)
    eligibility_trace = cp.zeros(grad_shape, dtype=cp.float32)

    args = (spike_indices, spike_times, exp_tau_s, exp_tau, pre_dir_derivatives, weights, perturbations, cp.bool(is_output),
            c, tau, max_simulation, max_n_pre_spike, max_n_post_spikes, n_pre_neurons,
            n_spikes, post_spike_times, post_dir_derivatives, trace_a, trace_b, last_dt_dw, eligibility_trace)
